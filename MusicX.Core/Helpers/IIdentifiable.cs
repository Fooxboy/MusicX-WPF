...
public interface IIdentifiable
{
    ...
    {
        if (data.Block != null) ProcessBlock(data.Block, data);
        ...
        if (data.Playlist != null) FillPlaylistOwnerName(data.Playlist, data);
        
        if (data.Section != null) ProcessSection(data.Section, data);
    }
    ...
    private static void RemoveEmptySeparators(Section section)
    {
        ...
        if (snippetsBannerIndex < 0) return;
    ...
    private static void MergeRefBlocks(Section section)
    {
        for (var i = 0; i < section.Blocks.Count; i++)
        {
            ...
            if (block is not { DataType: "action", Layout.Name: "horizontal_buttons" }) continue;
            ...
            if (refBlockIndex < 0 || block.Actions[0].Action.Type != "open_section") continue;
